...
const GameOver = () => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-10 z-50 pb-64">
            <div className="bg-white bg-opacity-90 p-4 rounded-md shadow-lg flex flex-col items-center gap-6">
                <p className="text-xl">Oyun Bitti! Kelime şu:</p>
                <p className="text-xl">{currentWord}</p>
                <button
                    className="px-4 py-2 bg-opacity-80 bg-gray-500 text-white rounded hover:bg-gray-600 focus:outline-none"
                    onClick={handleReset}
                >
                    Yeniden Dene
                </button>
